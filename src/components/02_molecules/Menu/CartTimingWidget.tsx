...
export default function CartTimingWidget({ defaultMinPrepMinutes = 45, onTimingChange }: Props) {
  ...
  // What to show in the <input type="datetime-local">
  const inputValue = React.useMemo(() => {
    if (!timing.scheduledAt) return minLocal;
    const d = new Date(timing.scheduledAt); // parsed as LOCAL, which is what we want
  ...
  return (
    <div style={{ border: "1px solid #eee", borderRadius: 8, padding: 12 }}>
      ...
      <label style={{ display: "block", marginBottom: 6 }}>
        <input type="radio" checked={timing.asap} onChange={selectASAP} /> ASAP (about {defaultMinPrepMinutes} min)
      </label>

      <label style={{ display: "block", marginBottom: 8 }}>
        <input type="radio" checked={!timing.asap} onChange={selectSchedule} /> Schedule a time
      </label>
