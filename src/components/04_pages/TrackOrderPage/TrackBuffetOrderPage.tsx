...
import { trackBuffetOrder } from "../../../services/buffetOrders";
...
export default function TrackBuffetOrderPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      setError(undefined);
      const res = await trackBuffetOrder(orderId, email);
      setData(res);
    } catch (e: any) {
      setError(e?.response?.data?.message || "Could not find your buffet order.");
    } finally {
      setLoading(false);
    }
  }, [orderId, email]);
      ...
      <Paper elevation={0} sx={{ p: 3, border: "1px solid rgba(11,45,36,.12)" }}>
        ...
        <Box sx={{ display: "grid", gap: 0.5 }}>
          {(data.orderItems || []).map((it) => (
            <Typography key={String(it.buffetItemId)} variant="body2" sx={{ color: AK_DARK }}>
              {it.quantity} × {it.name} — CHF {Number(it.unitPrice || 0).toFixed(2)}
            </Typography>
          ))}
        </Box>
